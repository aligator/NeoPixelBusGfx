...
/**
 * @brief Template for using NeoPixel matrices with the GFX graphics library by
 * making use of NeoPixelBus. It contains the logic for the implementations of
 * NeoPixelBusGfx and NeoPixelBrightnessBusGfx to avoid code duplication.
 * T_NEO_PIXEL_BUS should be the specific NeoPixelBus class. (e.g. NeoPixelBus
...
public:
  ...
  NeoGfx(int w, int h,
         NeoPixelBus<T_COLOR_FEATURE, T_METHOD> *neoPixelBusInstance)
      : matrixWidth(w), matrixHeight(h), remapFn(NULL),
        neoPixelBus(neoPixelBusInstance) {}
   ...
   */
  void drawPixel(int16_t x, int16_t y, uint16_t color, uint16_t _width,
                 uint16_t _height, uint8_t rotation, int16_t WIDTH,
                 int16_t HEIGHT) {
    ...
    ((T_NEO_PIXEL_BUS *)neoPixelBus)
        ->SetPixelColor(tileOffset + pixelOffset,
                        passThruFlag ? passThruColor
                                     : RgbColor(HtmlColor(expandColor(color))));
  }
  ...
  void fillScreen(uint16_t color) {
    ...
    for (i = 0; i < n; i++)
      ((T_NEO_PIXEL_BUS *)neoPixelBus)->SetPixelColor(i, c);
  }
   ...
   */
  void setRemapFunction(uint16_t (*fn)(uint16_t, uint16_t)) { remapFn = fn; }
